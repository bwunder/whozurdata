...
var initStoreNames = function (data) {
  data.storeNames = new Array;
  fs.readdirSync(__dirname.replace('routes', 'stores')).forEach(function (file) {
    data.storeNames.push(path.basename(file, '.js'));
  });
//  data.domain.emit('test', {'function': 'initStoreNames', 'storeNames': data.storeNames});
};

// stores, rendering and routing are loaded at startup but file can also be edited on the fly?
var importStores = function(data) {
  try {
    // revert to existing on error
    var saved = data;
    initStoreNames(data);
    data.stores = [];
    data.storeNames.forEach(function(name) {
      try {
        data.stores.push(require('../stores/' + name)); 
        data.stores[data.storeNames.indexOf(name)].store.setVersion();
//        data.domain.emit('test', {'function': 'importStores', 'store': name});
      }
      catch (e) {
          throw(e);
//        data.domain.emit('test', {'function': 'importStores', 'store': name, 'NODB': e});
      }
    });
  }
  catch (e) {
    throw(e);
//    data.domain.emit('error', e);
//    data = saved;  // halleluya
  }
};


/// ?? can this be called later after on the fly changes to the file???
///var importRender = function(data) {
///  render = require('./page/render');
///  data.domain.emit('test', {'function': 'importRender', 'result': data.render });
///};

///var importRoutes = function(data) {
///  routes = require('./urRoutes');
///  data.domain.emit('test', {'function': 'importRoutes', 'result': data.routes});
///};
...
var verifySchema = function(data) {
  var verified = true, 
  ...
  var asExpected = function(actualArray, expectedArray) {
    expectedArray.forEach( function (element) {
      if (verified & actualArray.indexOf(element) < 0) {
        verified = false;
        data.domain.emit('app', {'function': 'asExpected', 
                                        'array': actualArray, 
  ...
  data.domain.emit('app', {'function': 'verifySchema', 'result': verified});
  return verified;
};
